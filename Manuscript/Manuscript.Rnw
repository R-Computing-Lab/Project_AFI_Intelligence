...
@
<<setup_Data_9,include=FALSE,cache=FALSE>>=
...
@
<<setup_Data_10,include=FALSE,cache=FALSE>>=
...
@
<<setup_Data_11,include=FALSE,cache=FALSE>>=
# Import Data for age 10
## First Born Child
...
@
<<setup_summarystats,include=FALSE,cache=FALSE>>=
# Import Data
...
@
<<setup_Reliability,include=FALSE,cache=FALSE>>=
# Reliability 
## AFI G1
###Import Data
...
@
<<measurement,include=FALSE,cache=FALSE>>=
# G1
## mplus
### Run
...
@
<<results_regression,echo=FALSE, comment=NA, include=FALSE,cache=FALSE>>=
...
@
<<setup_tables,include=FALSE,cache=FALSE>>=
# Summary Stats G1
## Manage data
### Import
...
\begin{center}
<<plot_afi_by_race_sex,fig=TRUE,echo=FALSE,cache=FALSE,echo=FALSE, fig.height=5*.8, fig.width=5,out.width=".65\\paperwidth",warning=FALSE>>=
# Data Prep
...
<<calculationalphadif,cache=FALSE,include=FALSE>>=
# Intelligence
sigmax = sqrt(1.1881)
sigmay = sigmax
rxx = .87
...
\label{plot_gen1_afi}
\begin{center}
<<plot_gen1_afi,fig=TRUE,echo=FALSE,cache=FALSE,echo=FALSE, fig.height=5*.8, fig.width=5,out.width=".8\\paperwidth",warning=FALSE>>=
library(ggplot2)
library(gtable)
library(grid)
library(gridExtra)
...
##############
ggplot(dsD, aes(y=AFI_G1_S1, x=Intel_G1_S1,colour=(DIF_G))) + 
...
\label{plot_gen2_afi}
\begin{center}
<<plot_gen2_afi,fig=TRUE,echo=FALSE,cache=FALSE,echo=FALSE, fig.height=5*.8, fig.width=5,out.width=".8\\paperwidth",warning=FALSE>>=
...
ggplot(ds2D, aes(y=AFI_G2, x=Intel_G2,colour=(Intel_G1))) + 
  ...
  scale_color_gradientn(values=values,rescaler = function(x, ...) x, oob = identity,name="Maternal\n AFQT",colours=shading)+
  theme_min(size=10) +
     theme(plot.title=element_text(size=16,face="bold"))+ 
  ylab("Age at First Intercourse") +
  xlab("General Cognitive Ability") #+
#ggtitle("Generation 1:\n Age at First Intercourse vs. Intelligence")
